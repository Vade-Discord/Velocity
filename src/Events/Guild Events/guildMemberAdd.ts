    ...
    async run(guild, member) {
        if (await this.client.redis.get(`antiraid.${guild.id}`)) {
            const embed = new this.client.embed()
                ...
                .setColor('#F00000')
                .setDescription(
                    `You have been automatically kicked from **${guild.name}** due to the anti-raid module being active. This is automatically disabled after **1 hour**.`
                )
            ...
            const antiRaidTriggered = new this.client.embed()
                .setAuthor(`${tag} kicked due to Anti-Raid`, member.user.avatarURL)
        ...
        if (
            ...
            guildAutoRoleData &&
            guildAutoRoleData?.enabled &&
            guildAutoRoleData.roles?.length
        ) {
            await member.edit({
        ...
        if (inviteChannel) {
            ...
            const invite: Invite =
                invites.find(
                    (x) => gi?.find((e) => e.code === x.code).uses < x.uses && gi?.includes(x.code)
                ) ||
            ...
            if (!invite) {
                inviteChannel.createMessage(
                    `${member.mention} joined! Unable to locate who they were invited by.`
                );
            } else {
        ...
        if (welcomeChannel) {
            if (
                ...
                welcomeChannel.permissionsOf(this.client.user.id).has('sendMessages')
            ) {
                return welcomeChannel.createMessage(
                    guildData && guildData?.welcomeMessage
                        ? `${member.mention}, ${guildData?.welcomeMessage}`
