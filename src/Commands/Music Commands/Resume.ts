...
export default class ResumeCommand extends Command {
    ...
    async run(message, args) {
        ...
        if(!player || !player.paused) return message.channel.createMessage({ content: `The player doesn't seem to be paused.`, messageReference: { messageID: message.id }});
        player.pause(false);
        let embed = new this.client.embed()
            .setDescription(`${message.author.mention} ▶ resumed the music!`)
            .setColor('#ffffff');
